      ...
      <div className="flex items-center">
        ...
        <Link to="/login">
          <div className="flex items-center cursor-pointer space-x-2 border border-orange-400 p-2 rounded-lg hover:bg-orange-100 duration-300">
            ...
            {!user.currentUser ? <span className="text-[#e455c5] hover:text-pink-600 font-semibold">
              Login
            </span> :
            
            <Link to="/myaccount"> 
              <span  className="text-[#e455c5] hover:text-pink-600 transition duration-300 font-semibold">{user.currentUser.name}</span>
            </Link>
            }
          </div>
        </Link>
